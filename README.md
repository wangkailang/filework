...
> 本地优先的通用 AI 工作区 Agent，把对话、文件操作、命令执行、Git 协作、MCP 工具和可扩展技能放进同一个桌面工作流。

Workspace Agent 是一个基于 Electron + React 的桌面应用。它在用户选定的工作区内运行，默认具备读写文件、浏览项目结构、执行 shell 命令、管理 Git 工作流、调用外部 MCP 工具和运行内置 / 自定义技能的能力。文件管理是它的重要场景，但不是唯一身份；它更接近一个可审计、可扩展、可由用户控制的本地项目执行代理。

应用采用 local-first 设计：会话、配置、缓存、任务状态和工作区记忆都保存在本机；只有在用户配置模型服务后，请求才会发往对应的 LLM API。对敏感操作，Agent 会通过审批门控让用户确认，适合在真实项目目录中承担分析、整理、改写、调试、自动化和协作类任务。

> ℹ️ 仓库 / npm 包名 / preload 命名空间 / 用户数据目录仍沿用历史标识符 `filework`，未做技术性重命名以避免破坏已有用户数据与 macOS 代码签名身份。仅 UI / 文档层面统一为 "Workspace Agent"。

## Core Capabilities

- 🤖 **工作区执行代理** — 默认具备读 / 写 / 列文件、运行 shell、向用户追问和按计划推进任务的能力
- 🧩 **技能扩展系统** — 文件整理、PDF / Excel / Word 解析、报告生成等能力以技能形式内置，也可安装项目级或用户级技能
...
- 🔒 **本地优先** — 会话历史 / 配置 / 缓存 / 工作区记忆全部在本地，模型 API 是唯一可选的外部依赖
