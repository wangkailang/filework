          ...
          if (sql.includes("automation_runs")) {
            return {
              sql: state.hasTriageStatus
                ? "CREATE TABLE automation_runs (status TEXT CHECK(status IN ('needs_action')), triage_status TEXT, needs_action_reason TEXT, chat_session_id TEXT, assistant_message_id TEXT, task_id TEXT)"
