 ...
 * `GitHubWorkspace` / `GitLabWorkspace` extend it with clone + auth so the
 * agent can drive git through `runCommand` against an authenticated remote.
 *
...
/**
 * Narrow host-side helper exposed by git-backed workspaces. The agent
 * never sees this — it drives git through `WorkspaceExec.run()` against
 * `git` / `gh` / `glab` CLIs. The host uses it for UI affordances like
 * the branch picker, where reusing the already-authenticated clone is
 * cheaper than spinning up a fresh `git fetch` from the renderer side.
 */
export interface WorkspaceSCM {
  /** Symbolic name of the currently checked-out branch. */
  currentBranch(): Promise<string>;
  /**
   * Fetch from origin, then switch the working tree to `branch`. Creates
   * a local tracking branch when needed. Refuses to operate on a dirty
   * tree — the caller must commit or discard first. Returns the prior
   * branch so the UI can offer "switch back" affordances.
   */
...
export interface Workspace {
  ...
  /** Present on git-backed workspaces; absent on LocalWorkspace. Host-only. */
