...
async function migrateLegacyBlock(workspace: Workspace): Promise<void> {
  ...
  // 从 AGENTS.md 移除块,压掉多余空行,写回干净内容。
  const cleaned = content
    .replace(re, "")
    .replace(/\n{3,}/g, "\n\n")
    .trim();
  await workspace.fs.writeFile("AGENTS.md", cleaned ? `${cleaned}\n` : "");
...
export async function getWorkspaceMemoryInfo(
  ...
  const parts: string[] = [];
  if (humanContent) parts.push(humanContent);
  if (agentMemory) parts.push(agentMemory);
  let combined: string | null = parts.length ? parts.join("\n\n") : null;
  if (combined && combined.length > MAX_MEMORY_CHARS) {
    combined = `${combined.slice(0, MAX_MEMORY_CHARS)}\n\n…(workspace memory truncated)`;
  }
