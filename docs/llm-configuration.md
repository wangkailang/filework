...
| `model` | 当前使用的模型 ID |
| `temperature` | 可选采样温度，留空时使用 provider 默认值 |
| `topP` | 可选 nucleus sampling 参数，留空时使用 provider 默认值 |
| `maxOutputTokens` | 可选单次输出 token 上限 |
| `reasoningEffort` | 可选推理强度，传给支持 reasoning effort 的 OpenAI 兼容模型 |
| `modality` | `chat`、`image` 或 `video`；`chat` 进入 AgentLoop，`image` / `video` 进入媒体运行时 |
...
每个 LLM 配置可以设置一组可选的高级生成参数。所有字段留空时都会使用
provider 或模型默认值：

| 参数 | 范围 | 运行时行为 |
| --- | --- | --- |
| `temperature` | `0` 到 `2` | 传给 AgentLoop 的 `streamText` 调用 |
| `topP` | `0` 到 `1` | 传给 AgentLoop 的 `streamText` 调用 |
| `maxOutputTokens` | 正整数 | 限制单次 `streamText` 输出 token |
| `reasoningEffort` | `none` / `minimal` / `low` / `medium` / `high` / `xhigh` | 当模型目录未明确标记为不支持 reasoning 时,合并到 OpenAI provider options |

这些参数会随配置保存到本地 `llm_configs` 表。聊天主路径、自动化 headless
运行和子 Agent 运行都会复用同一套配置，避免主任务和委派任务的模型行为不一致。
如果刷新到的模型目录明确标记当前模型不支持 reasoning，设置页会禁用推理强度
选择器，运行时也不会把 `reasoningEffort` 发给 provider，避免非 reasoning 模型因
多余参数返回 400。

## 运行时如何选择配置

聊天发送统一从 `ai:executeTask` 进入主进程。运行时选择 LLM 的流程是：

1. 根据用户选择的 `llmConfigId` 找到配置；如果没有传入，则找默认配置。
2. 如果用户显式选择了 `image` / `video` 配置，运行时先检查它是否健康：
   - `enabled !== false`
   - `lastCheckStatus === "success"`
   - `modelAvailable !== false`
3. 健康的 `image` 配置会调用媒体运行时生成图片，并通过 `ai:stream-media`
   推送 `ImagePart`；健康的 `video` 配置会创建 `media_jobs` 记录、启动 watcher，
   并通过 `ai:stream-media` 推送 `VideoJobPart`。
4. 其它情况进入 chat 选择流程，检查配置是否可用于聊天：
   - `enabled !== false`
   - `modality === "chat"`
   - `lastCheckStatus === "success"`
   - `modelAvailable !== false`
5. 如果用户选择的 chat 配置不可用，则按配置列表顺序回退到最近更新的健康 chat 配置。
6. 通过 `getModelAndAdapterByConfigId` 创建 AI SDK `LanguageModel`。
7. 将配置中的高级生成参数和 provider options 一并交给 `AgentLoop`。
8. 把模型交给 `AgentLoop` 执行聊天、工具调用、上下文压缩和反思逻辑。
...
| 图像 / 视频媒体运行时 | `src/main/ipc/media-runtime.ts` |
