...
| `tasks[].maxTurns` | — | 10 / researcher 10 | 子 agent 回合数上限 |
| `tasks[].maxTotalTokens` | — | 120000 / researcher 60000 | 子 agent 累计 token 上限 |
| `tasks[].maxWallMs` | — | 120000 / researcher 180000 | 子 agent 墙钟上限(毫秒) |
...
|---|---|---|---|
| 回合数 | `maxStepsPerTurn` | 10 / researcher 10 | `stopReason="max_steps"` |
| 累计 token(in+out) | `maxTotalTokens` | 120000 / researcher 60000 | `stopReason="token_budget"` |
| 墙钟 | `maxWallMs` | 120000 / researcher 180000 | `stopReason="wall_clock"` |
...
默认常量在 `src/main/core/agent/sub-agent-contract.ts`:`DEFAULT_SUB_AGENT_MAX_TURNS=10`、`DEFAULT_SUB_AGENT_MAX_TOTAL_TOKENS=120_000`、`DEFAULT_SUB_AGENT_MAX_WALL_MS=120_000`。`researcher` 的专用预算在 `src/main/ipc/agent-tools.ts` 中解析。

### 研究调用相位守卫

子 agent 的联网调研按 `发现 → 核验 → 提交` 三个相位推进,由 `ResearchLoopGuard` 在工具注册层执行,不依赖模型自觉停止:

- 发现阶段通常在 2 次聚焦搜索且覆盖至少 5 个 URL / 2 个 host 后切到核验;同参数重复或相似低增量搜索也会提前切相位。
- 单个子任务最多实际执行 4 次 `webSearch` 和 4 次 `webFetch` / `webFetchRendered` / `webScrape`;过期调用返回成功的 `skipped` 相位切换结果,不制造错误重试循环。
- 核验阶段累计 3 个不同且正文可用的来源后切到提交;提交阶段用显式 tool choice 强制 `submitSubagentResult`,并用 terminal tool 立即收口。完整工具名仍保留在 SDK 解析集合中,滞后的搜索/抓取会由执行前守卫转换成成功的 `skipped`,不会实际联网或产生 `AI_NoSuchToolError`。
- 较旧搜索结果会压缩成 evidence ledger,避免重复片段持续挤占上下文。
...
渲染层(`useStreamSubscription.ts`)按 `parentTaskId === 当前主任务` 过滤,`batchId` 定位 `SubagentMessagePart`、`childTaskId` 定位卡内某一行。组件 `SubagentCard.tsx` 是一张可折叠卡:标题显示「N 子任务 · 并发 M · 完成 X/N · 可用 Y/N」,每行显示 goal、状态徽章(进行中/完成/部分可用/失败/截断)、步数、token。`usable_partial` 计入可用证据,但单独标记为需要继续核验。
...
- 联网研究调用提升为阶段摘要:直接显示已完成搜索数、去重且正文达标的核验来源数、提交状态、阶段切换与失败数。逐条 `webSearch` / 抓取 / `submitSubagentResult` 仍可追溯,但默认收进「研究调用明细」,避免十几行工具事件淹没结论。
