...
| `tasks[].outputFormat` | — | `summary` | `summary` / `json` / `answer` / `patch`。`json` 时会按约定提取 artifacts |
| `tasks[].allowedTools` | — | 继承父集 | 子 agent 工具白名单,**只能是你的子集** |
| `tasks[].allowedSkills` | — | 继承父集 | 子 agent skill 白名单,**只能是你的子集** |
| `tasks[].maxTurns` | — | 10 | 子 agent 回合数上限 |
| `tasks[].maxTotalTokens` | — | 60000 | 子 agent 累计 token 上限 |
| `tasks[].maxWallMs` | — | 120000 | 子 agent 墙钟上限(毫秒) |
| `concurrency` | — | 3 | 最大并发子任务数(钳到 1–4) |
| `failFast` | — | false | 首个失败的子任务取消其余 |

返回给模型的 tool-result:`{ success, batchId, reports: [{ goal, status, summary, artifacts, usage, error }] }`。模型在下一回合综合这些报告答复用户。
...
|---|---|---|---|
| 回合数 | `maxStepsPerTurn` | 10 | `stopReason="max_steps"` |
| 累计 token(in+out) | `maxTotalTokens` | 60000 | `stopReason="token_budget"` |
| 墙钟 | `maxWallMs` | 120000 | `stopReason="wall_clock"` |
...
常量在 `src/main/core/agent/sub-agent-contract.ts`:`DEFAULT_SUB_AGENT_MAX_TURNS=10`、`DEFAULT_SUB_AGENT_MAX_TOTAL_TOKENS=60_000`、`DEFAULT_SUB_AGENT_MAX_WALL_MS=120_000`。
