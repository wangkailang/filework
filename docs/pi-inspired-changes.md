...
| **M17** | GitHub PR 行级评论 edit / delete | #52 |
...
### 17. GitHub PR 行级评论 edit / delete(M17)

**PI 思路**:M10 让 Agent 留得了 inline 评论,M15 把 review summary 的生命周期补完了,但**单条 inline 评论本身**还是不可变的。Agent 写错一行 review,只有两种粗放选择:再发一条评论纠正(噪音大),或 dismiss 整个 review(连带把判定 + summary + 其他几条评论一起作废,杀鸡用牛刀)。M17 把这个颗粒度补上 —— 只改 / 删一条 inline。

**filework 实现**:

- 新类型 `PullRequestReviewCommentSummary`(`types.ts`):窄投影 `{id, reviewId, author, path, line, body, url, createdAt, updatedAt}`。`line: null` 透传 GitHub 的「outdated comment」标记,`reviewId: null` 透传非 review 直挂的 diff 评论 —— Agent 自己读了再决定怎么处理。
- 三个 GitHub-only 可选 SCM 方法:
  - `listPullRequestReviewComments?({number})` → GET `/pulls/:n/comments?per_page=100`。读取一个 PR 上所有 inline 评论,Agent 用来定位 `commentId`。
  - `editPullRequestReviewComment?({commentId, body})` → PATCH `/pulls/comments/:id`,只换 body。
  - `deletePullRequestReviewComment?({commentId})` → DELETE `/pulls/comments/:id`(204,空 body)。
- 三个 GitHub 工具:`githubListPullRequestReviewComments`(safe)、`githubEditPullRequestReviewComment`(destructive)、`githubDeletePullRequestReviewComment`(destructive)。审批文案:`编辑 PR 行评论 #N: <body 前 60 字>` / `删除 PR 行评论 #N`。
- 加入 `ALWAYS_PROMPT_TOOLS`:同 `commentPullRequest` / `dismissReview` / `editReviewBody` 一样,公开内容的 mutation 不进白名单,task 内每次都弹卡。
- 工程底层加了两个新私有 helper:`ghPatch<T>(path, body, label?)` 镜像 `ghPut`;`ghDeleteNoBody(path, label?)` 镜像 `ghPostNoBody`(GitHub `DELETE /pulls/comments/:id` 返回 204 + 空 body,不能走 `res.json()`)。

**关键决策**:

- **为什么是「列表 + edit + delete」三件套,而不是把 comment ids 折进 M10 的 `reviewPullRequest` 返回 shape?** GitHub 自己的 API 模型就是 `comment_id` 作主键,不嵌套在 review 下:`/repos/{o}/{r}/pulls/comments/{comment_id}` —— 路径里没有 `{review_id}`。让 Agent 走「list → 拿 id → edit/delete」更贴合上游契约,而且 Agent 可以编辑自己**过去任何一次** review 留下的评论,不仅限于刚刚那次。
- **方法名为何是 `*PullRequestReviewComment` 而不是 `*PullRequestComment`?** GitHub 一个 PR 上有两套评论:issue-style 的 `/issues/:n/comments`(M6 的 `commentPullRequest` 已覆盖)和 review inline 的 `/pulls/comments/:id`(M17)。命名直接区分,免得 Agent 调错。
- **GitLab 侧不实现**:GitLab 的对等 endpoint 是 `PUT/DELETE /projects/:id/merge_requests/:iid/discussions/:disc/notes/:note`,需要 `discussion_id` + `note_id` 两段 id。M10 当时 `reviewMergeRequest` 只回传第一条 discussion id,没存全集 —— 补 GitLab 之前要先扩展 M10 的返回 shape,这条留待 M18。
- **edit + delete 都做,不取一弃一**:只 edit 没 delete,Agent 想撤回评论得改成空字符串(GitHub 不允许);只 delete 没 edit,改一个错别字也得 delete + 重发,粒度倒退。
- **`line: null` / `reviewId: null` 不过滤**:outdated 评论(线已经从 HEAD 消失)和 diff 直挂评论(没有 review 父对象)Agent 都可能想删。投影里直接透传,不做隐藏。
- **403 / 404 如实抛出**:GitHub 只允许评论作者本人或 repo admin 编辑/删除自己的评论,失败时 `403 Must be authenticated as the author`。实现里不去捕获或包装,Agent 读到原文自己适配。

至此 GitHub 侧的 PR review 能力闭环 —— 创建 review、写多行 inline、改 review summary、dismiss verdict、改 / 删单条 inline。`PR review` 这个完整生命周期的所有操作,Agent 都能走通。
...
- **GitLab MR 行级评论 edit / delete**:M17 已覆盖 GitHub 侧;GitLab 对等需要先扩展 M10 `reviewMergeRequest` 返回 shape(目前只回传第一条 discussion id,补齐之前要保留 `{discussionId, noteId}[]` 全集),再走 `PUT/DELETE /projects/:id/merge_requests/:iid/discussions/:disc/notes/:note`。
