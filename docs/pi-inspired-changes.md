...
- **GitHub combined check-runs**:覆盖 CircleCI 等第三方 check —— M8 只看 GitHub Actions runs,跨 CI 平台仍需统一接入点。
- **PR 行级评论 / review**:目前 `commentPullRequest` 只支持会话级评论,行级 review API 单独路径。
