...
- **GitLab createPipeline + watch**:GitHub workflow_dispatch / rerun 都进了 watcher,GitLab 主动触发还差最后一块 —— `POST /projects/:id/pipeline?ref=…&variables=…` 装入 SCM,watcher 直接复用 M12 的 `subscribe`(GitLab 同步返回 pipeline JSON,不需要 M13 的 resolve 重试)。下一轮重点。
